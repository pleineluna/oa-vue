    ...
    <el-form :model="dataForm"
      ...
      <el-form-item label="日期"
        ...
        <el-date-picker v-model="dataForm.date"
                        ...
                        clearable="clearable"
                        @change="loadPlaceList()"></el-date-picker>
      ...
      <el-form-item label="地点"
                    prop="place">
        <el-select v-model="dataForm.place"
                   placeholder="会议室"
                   size="medium"
                   style="width:34.5%"
                   clearable="clearable">
          <el-option v-for="one in placeList"
                     :key="one"
                     :value="one"
                     :label="one"></el-option>
        </el-select>
      ...
      <el-form-item label="成员"
                    prop="members">
        <el-transfer v-model="dataForm.members"
                     :data="users"
                     :titles="['员工', '参会人']"
                     filterable
  ...
  methods: {
    init: function (id) {
      ...
      that.$nextTick(() => {
        ...
        that.$axios.get('/sys/user/searchAllUser')
          .then(resp => {
            let temp = [];
            for (let one of resp.data.data) {
              temp.push({ key: one.id, label: one.realname });
            }
            that.users = temp;
          });
      });
    ...
    loadPlaceList: function () {
      ...
      that.$axios.post('meeting_room/searchFreeMeetingRoom', data)
        .then(resp => {
          that.placeList = resp.data.data;
        });
    },
    dataFormSubmit: function () {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          that.$axios.post("meeting/insert", data)
            .then(resp => {
              if (resp.data.data == 1) {
                that.visible = false
                that.$message({
